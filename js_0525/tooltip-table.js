...
$(window).on('resize load', debounce(() => {
    ...
    $(mobileTooltips).each((i, element) => {
      ...
      <span class="tippy-tooltip-text-span">${aLinkText.trim()} <br> ${tooltipAdditionalText.trim()}</span>`;
        ...
        if ((!isCurrentElementFlagIcon && assetSpanId && tooltipAdditionalText.length) || (!isDesktop && !isCurrentElementFlagIcon && assetSpanId && element.offsetWidth < element.scrollWidth) || (!isDesktop && !isCurrentElementFlagIcon && assetSpanId && element.offsetWidth <= 31)) {
            var tippyConfig = {
        ...
        //show type when a row does not fit in a cell on the desktop
        const maximumNumberOfCharacters=43
        var isShowTipsOnDesktop = aLink.text().length >= maximumNumberOfCharacters;

        if (isShowTipsOnDesktop && isDesktop) {
            var tippyConfig = {
                triggerTarget: parentTd[0],
        ...
        if ($(element).hasClass("nowrap-text") &&
            !isDesktop &&
